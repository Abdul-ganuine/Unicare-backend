...
// get all the students in the system
...
const updateUserDetails = asyncWrapper(async (req, res) => {
  ...
  const id = await Users.findOne({ _id: userId });
  const {
    username,
    schoolVoda,
    email,
  ...
  const currentDetails = await studentsApi.findOne({ id: id.student_id });
  ...
  if (
    ...
    updateUsername = await Users.findOne({ student_id: id.student_id });
  }
...
const postComments = asyncWrapper(async (req, res) => {
  const { comments, _id, userId } = req.body;
