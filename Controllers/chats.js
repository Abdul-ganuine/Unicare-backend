...
const sendMessage = asyncWrapper(async (req, res) => {
  ...
  if (voiceNote) {
    chat.messages.push({ sender: id, voiceNote });
    await chat.save();
  }

  if (vid) {
    chat.messages.push({ sender: id, vid });
    await chat.save();
  }

  if (aud) {
    chat.messages.push({ sender: id, aud });
    await chat.save();
  }

  if (img) {
    chat.messages.push({ sender: id, img });
    await chat.save();
  }

  if (pdf) {
    chat.messages.push({ sender: id, pdf, pdfSize, pdfOriginalName });
    await chat.save();
  }
